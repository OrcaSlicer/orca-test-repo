...
_E_LINE = re.compile(r"^G(92|[0-3])\s[^;\n]*?\bE(-?(?:\d+\.?\d*|\.\d+))", re.M)
...
def e_sum(text: str) -> float:
    ...
    total, last = 0.0, 0.0
    # G2/G3 carry the arc's extrusion exactly as G1 does, and arc fitting is on in
    # the shipped profiles, so leaving them out drops a large share of the total.
    # OrcaSlicer writes E without a leading zero ("E.04213"), so accept ".123" too.
    for m in _E_LINE.finditer(body):
        e = float(m.group(2))
        if m.group(1) == "92":
            # explicit reset of the extruder origin; only meaningful in absolute mode
            last = e
            continue
        if relative:
            if e > 0:
