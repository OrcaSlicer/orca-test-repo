...
def exit_code(result, ctx, equals=None, one_of=None, not_equals=None):
...
def file_not_contains(result, ctx, path, text):
    """The file exists and does not contain `text`. A missing file fails: the
    check is meaningless if the run never produced the output it inspects."""
    p = Path(str(path).format(**ctx))
    if not p.exists():
        _fail(f"expected file to exist to check its content: {p}", result)
    if text in p.read_text(errors="replace"):
...
def gcode_config_value(result, ctx, key, equals, path="{outputdir}/plate_1.gcode"):
    ...
    if isinstance(equals, list):
        # element-for-element, including length: a shorter or padded vector is a finding
        expected = [str(x) for x in equals]
        got_l = got if isinstance(got, list) else [got]
        ok = len(got_l) == len(expected) and all(sc._num_eq(a, b) for a, b in zip(got_l, expected))
    elif isinstance(got, list):
        # a scalar expectation on a vector key means every element reads that way,
        # so a zero-filled tail cannot hide behind element 0
        expected = str(equals)
        ok = bool(got) and all(sc._num_eq(a, expected) for a in got)
    else:
        expected = str(equals)
        ok = sc._num_eq(got, expected)
    if not ok:
        _fail(f"G-code config {key!r}: expected {expected!r}, got {got!r}", result)
...
def _is_zero(v) -> bool:
    if v in ("nil", "", None):
        return True
    try:
        return float(str(v).rstrip("%")) == 0.0
    except ValueError:
        return False


def gcode_config_vector_nonzero(result, ctx, key, path="{outputdir}/plate_1.gcode"):
    """Every element of vector option `key` in the G-code config block is non-zero
    -- catches a partial CLI override that silently zero-fills the rest. An absent
    key fails: nothing can be asserted about a value the block does not carry."""
    cfg = _gcode_config(result, ctx, path)
    if key not in cfg:
        _fail(f"G-code config block has no key {key!r}", result)
    vals = cfg[key]
    vals = vals if isinstance(vals, list) else [vals]
    zeros = [i for i, v in enumerate(vals) if _is_zero(v)]
    if zeros:
...
def gcode_z_above_bed(result, ctx, min=0.0, path="{outputdir}/plate_1.gcode"):
    ...
    if sc.BLOCK_START in t and sc.BLOCK_END in t:
        ...
        body = t
    # OrcaSlicer writes coordinates without a leading zero ("Z.6", "Z-.2"), and a
    # below-bed Z is exactly what this check exists to see, so accept both spellings.
...
def gcode_body_count(result, ctx, text, equals=None, at_least=None, path="{outputdir}/plate_1.gcode"):
...
def threemf_member_contains(result, ctx, path, member, text):
...
def threemf_member_absent(result, ctx, path, member_prefix):
...
def result_json_field(result, ctx, field, equals=None):
