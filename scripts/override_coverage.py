...
def main() -> int:
    ...
    origin = r.get("surface_origin", "")
    origin = ("the committed snapshot" if origin.endswith("cli_surface_full.json")
              else "enumerated from an OrcaSlicer source checkout")
    print(f"# Override-sweep coverage\n\nGenerated by `scripts/override_coverage.py` from a sweep against "
          f"**{r.get('build', 'an unrecorded build')}**: {r['invocations']} CLI invocations, "
          f"{len(r['probes'])} options probed, option surface {origin}.\n")
    print("Regenerate after a sweep against a newer build:\n"
          "`pytest test_cli_overrides.py --orca-bin <bin> --orca-source <checkout>` then\n"
          "`python scripts/override_coverage.py > cases/_snapshots/override_coverage.md`.\n")
