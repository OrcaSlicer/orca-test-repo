...
def _build_id(orca_bin: Path) -> str:
    """Name the build the way baseline.json does: the version the binary
    reports, plus the commit of the source checkout when one is known. The
    binary's own path says where the sweep ran, not what it tested, so it must
    not end up in the committed coverage snapshot."""
    import surface as surface_mod

    out = subprocess.run([str(orca_bin), "--help"], capture_output=True, text=True)
    version = (out.stdout.splitlines() or ["unknown build"])[0].rstrip(":")
    src = surface_mod.source_dir()
...
def _bisect(keys, runner):
    ...
    if len(keys) == 1:
        ...
        return out
    mid = len(keys) // 2
    out.update(_bisect(keys[:mid], runner))
    out.update(_bisect(keys[mid:], runner))
    return out
...
def override_results(orca_bin, seeded_data_dir, tmp_path_factory, request):
    ...
    def failure_kind(rc):
        """Bucket for a run that produced no usable output. Exit 0 is the
        interesting one: the CLI reported success and wrote nothing, which is a
        silent drop, not the validation rejection a non-zero exit represents."""
        if rc is None:
            return "hang"
        if rc < 0:
            return "crash"
        return "silent_drop" if rc == 0 else "rejected"
    ...
    for i in range(0, len(keys), BATCH):
        merge.update(_bisect(keys[i:i + BATCH], merge_runner))
    ...
    report = {"probes": probes, "effect_probes": effect_probes, "skipped": skipped,
              ...
              "invocations": sweep.n,
              "build": _build_id(orca_bin), "binary": str(orca_bin),
              "surface_origin": surface_mod.load()["origin"]}
