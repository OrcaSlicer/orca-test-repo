...
def pytest_collection_modifyitems(config, items):
    """Mark `status: open` cases xfail -- see test_cases.py docstring."""
    for item in items:
        ...
        if case["status"] == "open":
            item.add_marker(
                # strictness comes from pytest.ini (xfail_strict) so run_test.py
                # can switch it off for OrcaSlicer's CI; see TESTING_STRATEGY.md
                pytest.mark.xfail(
...
def run_orca(orca_bin, tmp_path):
    def _run(args, cwd=None, timeout=300):
        ...
        return RunResult(argv, proc.returncode, proc.stdout, proc.stderr)
