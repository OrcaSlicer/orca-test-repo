...
TESTING_STRATEGY.md "Settings import"):
  ...
  * a preset handed to the CLI must be *self-contained*: one that still
    declares `inherits` is only accepted if it is already part of the
    datadir's loaded bundle. The fixtures are authored thin and flattened by
    presets.py before the run (see `_cli_preset`), so what they assert is
    still "these user keys survive import";
...
def _resolve(ref: str, datadir: Path) -> Path:
    """The preset as authored -- what the recipe's expectations are read from."""
...
def _cli_preset(ref: str, datadir: Path, scratch: Path) -> Path:
    """The file the CLI is given. A fixture preset inherits from a shipped
    profile, and only a self-contained preset can be loaded from outside the
    datadir, so it is flattened against the profiles under test first."""
    src = _resolve(ref, datadir)
    if ref.startswith("fx:"):
        return presets.flatten_to(src, datadir, scratch / "presets")
    return src
...
def test_settings_roundtrip(recipe, run_orca, datadir, outputdir, model):
    ...
    scratch = outputdir.parent
    args = ["--datadir", datadir, "--outputdir", outputdir, "--allow-newer-file"]
    if recipe.get("load_settings"):
        args += ["--load-settings", ";".join(str(_cli_preset(r, datadir, scratch)) for r in recipe["load_settings"])]
    if recipe.get("load_filaments"):
        args += ["--load-filaments", ";".join(str(_cli_preset(r, datadir, scratch)) for r in recipe["load_filaments"])]
