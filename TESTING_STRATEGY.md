...
The strictness is scoped to *this repo's* runs (`pytest.ini`, used by
`run_tests.sh`, plain `pytest` and the nightly `regression` job). The run
inside OrcaSlicer's CI goes through `run_test.py`, which turns it off
(`-o xfail_strict=false`): there an XPASS is listed in the job summary but
does not fail the build. Otherwise a PR that fixes a catalogued bug would go
red for fixing it, and flipping the case first would turn main red for every
other PR until the fix merged. The flow is therefore: the fix PR's CI shows
the XPASS and stays green; after it merges, this repo's next nightly fails
strictly on that case; one small PR here flips it to `fixed` and pins the
exit code or message the fix produces. **Getting this backwards -- writing checks that assert the current
...
model's metrics were captured on (the version string and, when the
source checkout is known, its commit); a legacy size-only
...
(the G-code parser's type map, the override sweep, the GUI-guard's action
list) reads one loader. If an OrcaSlicer source checkout is available
...
**No poison-value fuzzing here.** An earlier layer swept the same
enumeration with type-appropriate junk values asserting only `no_crash`. It
did its job -- it found the `nil`-into-a-non-nullable-vector abort on an
option nobody had tested by hand, now pinned by three `cases/crash/`
entries -- but it belongs in OrcaSlicer's own unit tests, where
`DynamicPrintConfig::read_cli` can be driven directly over the whole option
table in milliseconds instead of one process launch per value. Value-parsing
crashes found there should still get a case here when they are reachable
through a real CLI invocation, since that is what this suite proves.

## Settings import / export

**Presets handed to the CLI must be self-contained.** `--load-settings` and
`--load-filaments` resolve a preset that declares `inherits` against the
presets already loaded from the datadir, and refuse it if it is not one of
them ("Preset was not found in the loaded bundle"); a preset with no
`inherits` skips that lookup and is accepted from anywhere. The fixtures are
authored thin -- a handful of keys plus `inherits` from a shipped profile,
which is what makes them readable as "these are the user's settings" -- and
`presets.py` flattens them against the datadir under test before the run, so
what reaches the CLI is self-contained and still matched to the profiles the
binary ships. Case files get the same thing through `{flat_fixtures}`.

The flattened copy is marked `from: system`, which is required rather than
cosmetic: the CLI takes a preset's *system identity* from its own name when
`from` is "system" and from `inherits` otherwise, so a flattened preset left
marked "User" has no system identity and nothing can declare itself
compatible with it. That is why a user process fixture names the user machine
fixture in its `compatible_printers`.
...
`-m cli_overrides`; the merge and G-code stages take ~30 s (129 invocations).

Result on main (September 2026): 628 options probed (112 skipped, 89 of
them because they are not part of the merged config at all -- SLA and
GUI-only keys -- the rest nullable or empty vectors and one dynamic enum);
**620 landed in the merged config and 617 of those in the G-code of a real
slice, none silently dropped**. Rejections, all validation: `spiral_mode`,
`use_firmware_retraction`, `default_junction_deviation`,
`machine_max_junction_deviation`, `wall_maximum_resolution` and the three
`filament_mixed_*` keys at merge; `bridge_line_width`, `filament_printable`,
`other_layers_print_sequence_nums` at slice time.
...
- an `XPASS` on an `open` case -- *good* news: that build's behavior now
  matches the case's desired-behavior checks, i.e. the bug got fixed. Inside
  OrcaSlicer's CI it is listed in the summary and the build stays green;
  this repo's nightly reports it as a strict failure. Either way, update
  that case's `status` to `fixed` (and tighten/confirm its checks) once the
  fix is on main.

A case's `status` describes OrcaSlicer **main**: the build OrcaSlicer's
own CI runs the suite against, and the build this repo's own `regression`
job tests (the nightly AppImage published from main, see below). One
reference for both consumers keeps a status meaning one thing. A release
tag can be tested on demand (the workflow's `orca_ref` input, or locally
with the release AppImage), but expect the strict-xfail gate to fire there
for every bug main has fixed since that release: that is drift from the
reference, not a defect in the suite.
...
Both CI paths run with two `pytest-xdist` workers (`-n 2`): on a 4-vCPU
runner that roughly halves the wall time (measured 3 m 23 s -> 1 m 51 s
pinned to four CPUs), and a third worker gains little because every slice
...
repo's workflow runs two jobs so that changes to the *suite itself* get
feedback without waiting on an OrcaSlicer build:
...
- `regression` -- every push/PR, nightly on a schedule, and on-demand via
  `workflow_dispatch` -- downloads the nightly Linux AppImage OrcaSlicer
  publishes from main (or a release tag given as `orca_ref`), extracts it
  (no FUSE needed, via `--appimage-extract`), clones the matching source
  tree for the option surface, and runs the full suite. The AppImage is
  driven through its `AppRun` launcher: the bare binary cannot find the
  libraries bundled alongside it, and `AppRun` execs the binary so exit
  codes and crash signals reach the suite unchanged. The schedule exists so
  upstream drift is caught even when nothing in this repo changed.

Both jobs currently target Linux only. Extending to macOS/Windows runners
means fetching the matching nightly asset (`.dmg` / portable `.zip`) for
that platform and is a documented extension point, not yet implemented here.
